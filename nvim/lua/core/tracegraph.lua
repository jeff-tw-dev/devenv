-- Recursive call-trace tree: with the cursor on a function, open a side
-- panel and expand caller-of-caller (incoming) or callee-of-callee
-- (outgoing) level by level, based on LSP callHierarchy.
-- Panel keys: o/<Tab> expand/collapse, <CR> jump to call site, gd jump to
-- definition, p preview, s switch direction, q close. Each expand issues
-- one more LSP request, so the tree is naturally lazy.
local M = {}
...
local function render()
  ...
  lines[2] = " o:expand <CR>:call site gd:def p:preview s:direction q:quit"
...
local function create_panel()
  ...
  vim.keymap.set("n", "o", toggle_expand, opts)
  vim.keymap.set("n", "<Tab>", toggle_expand, opts)
  vim.keymap.set("n", "<CR>", jump_to_callsite, opts)
  vim.keymap.set("n", "gd", jump_to_definition, opts)
  vim.keymap.set("n", "p", preview, opts)
  vim.keymap.set("n", "s", switch_direction, opts)
